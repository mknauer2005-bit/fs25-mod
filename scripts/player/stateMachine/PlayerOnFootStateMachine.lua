...
---A sub-state of the PlayerStateMachine, also being a state machine itself. This is used when the player is walking around on foot.
...
-- @param Player player The player who owns the state machine.
-- @return PlayerOnFootStateMachine instance The created instance.
...
---Fired when this state machine becomes the current state of the PlayerStateMachine. This usually happens once the player leaves a vehicle.
...
---The update function for when this state machine is the current state in the main player state machine. Updates the on-foot movement and on-foot state.
