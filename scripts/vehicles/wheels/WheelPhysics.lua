...
--#profile     RemoteProfiler.zoneBeginN("updateSteeringAngle")
    ...
    elseif self.versatileYRot and self.vehicle:getIsVersatileYRotActive(self.wheel) then
        if self.vehicle.isServer then
            steeringAngle = self:getVersatileYRotation(dt)
        end
...
function WheelPhysics:getVersatileYRotation(dt)
    if self.forceVersatility or self.hasGroundContact then
        ...
        if self.versatileYRotSpring ~= nil then
            ...
            local mass = self.mass * 1000

            local velocityX, velocityY, velocityZ = getVelocityAtLocalPos(self.wheel.node, self.positionX, self.positionY, self.positionZ)
            velocityX, velocityY, velocityZ = velocityX * mass, velocityY * mass, velocityZ * mass

            if self.versatileYRotLastVelocity == nil then
                self.versatileYRotLastVelocity = {velocityX, velocityY, velocityZ}
            end

            local invDt = 1 / (dt * 0.001)
            local velocityImpact = 0.2
            local forceX = (velocityX - self.versatileYRotLastVelocity[1]) * invDt * velocityImpact
            local forceY = (velocityY - self.versatileYRotLastVelocity[2]) * invDt * velocityImpact
            local forceZ = (velocityZ - self.versatileYRotLastVelocity[3]) * invDt * velocityImpact

            self.versatileYRotLastVelocity[1], self.versatileYRotLastVelocity[2], self.versatileYRotLastVelocity[3] = velocityX, velocityY, velocityZ

            local gravityForceY = 9.81 * mass
            local localX, _, localZ = worldDirectionToLocal(getParent(self.wheel.repr), -forceX, -forceY - gravityForceY, -forceZ)
            local localLength = MathUtil.vector2Length(localX, localZ)

            steeringAngle = steeringAngle % (2 * math.pi)

            local targetRotY = MathUtil.getYRotationFromDirection(localX, localZ) + math.pi
            targetRotY = targetRotY % (2 * math.pi)
            targetRotY = MathUtil.normalizeRotationForShortestPath(targetRotY, steeringAngle)

            local rotio = math.clamp((localLength / gravityForceY), 0, 1)

            local rotOffset = targetRotY - steeringAngle
            rotOffset = rotOffset * rotio

            local staticFriction = self.versatileYRotFriction[1]
            local dynamicFriction = self.versatileYRotFriction[2]

            local drive = self.versatileYRotSpring * rotOffset - self.versatileYRotDamping * self.versatileYRotSpeed

            if math.abs(self.versatileYRotSpeed) < 0.00001 then
                if math.abs(drive) <= staticFriction then
                    drive = 0
                    self.versatileYRotSpeed = 0
                else
                    drive = drive - math.sign(drive) * dynamicFriction
                end
            else
                drive = drive - math.sign(self.versatileYRotSpeed) * dynamicFriction
            end

            self.versatileYRotSpeed = self.versatileYRotSpeed + drive * dt
            steeringAngle = steeringAngle + self.versatileYRotSpeed * dt
