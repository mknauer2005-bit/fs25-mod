...
function PalletFiller.initSpecialization()
    ...
    schema:register(XMLValueType.ANGLE, basePath .. ".palletRow#yRotation", "Y Rotation of the pallet", 0)
...
function PalletFiller:onLoad(savegame)
    ...
    spec.palletRow.yRotation = self.xmlFile:getValue(basePath .. ".palletRow#yRotation", 0)
    ...
    for i=1, spec.palletRow.maxNumPallets do
        ...
        setRotation(palletSlot.jointNode, 0, spec.palletRow.yRotation, 0)
        ...
        or (data ~= nil and data.slotIndex == slotIndex) then
            ...
            if data ~= nil then
                ...
                if math.abs(yRot) < math.pi * 0.5 then
                    palletSlot.endQuaternion = {mathEulerToQuaternion(0, spec.palletRow.yRotation, 0)}
                else
                    palletSlot.endQuaternion = {mathEulerToQuaternion(0, spec.palletRow.yRotation + math.pi, 0)}
                end
...
function PalletFiller:verifyCombine(superFunc, fruitType, outputFillType, ...)
    ...
    for i, palletSlot in ipairs(spec.palletRow.palletSlots) do
        if palletSlot.object ~= nil and palletSlot.object.getFillUnitFillType ~= nil then
            if palletSlot.object:getIsSynchronized() then
