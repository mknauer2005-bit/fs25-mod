...
function FillUnit.initSpecialization()
    ...
    schema:register(XMLValueType.INT, fillUnitPath .. ".alarmTriggers.alarmTrigger(?)#direction", "Direction in which the trigger is active (0: any, 1: filling, -1: discharging)", 0)
...
-- @param integer fillTypeIndex currently ignored/unused
-- @param integer farmId currently ignored/unused
...
function FillUnit:updateAlarmTriggers(alarmTriggers, fillLevelDelta)
    for _, alarmTrigger in pairs(alarmTriggers) do
        local isActive = self:getAlarmTriggerIsActive(alarmTrigger)

        if alarmTrigger.direction ~= 0 then
            fillLevelDelta = fillLevelDelta or alarmTrigger.lastDeltaFillLevel
            if fillLevelDelta ~= nil then
                if math.sign(fillLevelDelta) ~= alarmTrigger.direction then
                    isActive = false
                end

                alarmTrigger.lastDeltaFillLevel = fillLevelDelta
            else
                isActive = false
            end
        end

        self:setAlarmTriggerState(alarmTrigger, isActive)
    end
end
...
function FillUnit:addFillUnitFillLevel(farmId, fillUnitIndex, fillLevelDelta, fillTypeIndex, toolType, fillPositionData)
    ...
    if fillUnit ~= nil then
        ...
        if self.isClient then
            self:updateAlarmTriggers(fillUnit.alarmTriggers, fillLevelDelta)
...
---Adds fill trigger
-- @param table trigger trigger
-- @param integer fillTypeIndex fillTypeIndex
...
function FillActivatable.new(vehicle)
    local self = setmetatable({}, FillActivatable_mt)

    self.vehicle = vehicle
    self.fillTypeIndex = FillType.UNKNOWN
    self.activateText = "<FillActivatable>"
