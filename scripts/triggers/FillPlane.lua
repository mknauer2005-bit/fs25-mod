...
-- @param float state alpha value for fill level [0..1]
...
function FillPlane:setFillType(fillTypeIndex)
    if self.loaded then
        if FillPlaneUtil.assignDefaultMaterialsFromTerrain(self.node, g_terrainNode) then
...
-- @param array colorScale a float array for r, g, b
