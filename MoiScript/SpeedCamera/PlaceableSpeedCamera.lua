...
function PlaceableSpeedCamera:onLoad(savegame)
    local spec = self["spec_FS22_Region.speedCamera"]
        ...
        if vehicle ~= nil and vehicle.spec_drivable ~= nil and vehicle.spec_enterable ~= nil then
            if onEnter then
                ...
                if vehicleSpeed > g_i18n:getSpeed(spec.speedLimit) and spec.count == 1 then
                    ...
                    if spec.showBlinkingWarning and g_i18n:getText("Warning_speedCamera") ~= nil then
                        g_currentMission:showBlinkingWarning(g_i18n:getText("Warning_speedCamera"))
                    end
