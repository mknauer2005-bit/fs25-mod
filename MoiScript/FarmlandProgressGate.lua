...
function FarmlandProgressMessageEvent:run(connection)
    ...
    if self.farmId == nil or self.farmId == 0 or self.farmId == localFarmId then
    ...
    if #failedMessages > 0 then
        ...
        if prefix ~= nil and prefix ~= "" then
            ...
            for _, requirement in ipairs(rule.requirements) do
                ...
                local requiredValue = requirement.required or 0
                if currentValue + 0.00001 < requiredValue then
                    table.insert(compactMessages, self:buildRequirementMessage(farmId, requirement, true))
                end
            end
            ...
            for _, soldRequirement in ipairs(rule.soldFillTypes) do
                ...
                local requiredValue = soldRequirement.amount or 0
                if currentValue + 0.00001 < requiredValue then
                    table.insert(compactMessages, self:buildSoldFillTypeMessage(farmId, soldRequirement, true))
                end
            end
            ...
            for _, metricRequirement in ipairs(rule.metricRequirements or {}) do
                ...
                local requiredValue = metricRequirement.required or 0
                if currentValue + 0.00001 < requiredValue then
                    table.insert(compactMessages, self:buildMetricRequirementMessage(farmId, metricRequirement, true))
                end
            end

            return true, string.format("Для покупки участка необходимо %s: %s", prefix, table.concat(compactMessages, "; "))
        end
...
function FarmlandProgressGate:showReasonForFarm(farmId, text)
    ...
    if self:isServer() and g_server ~= nil and self:isClient() then
...
function FarmlandProgressGate:installStatsHooks()
    ...
    manager.updateFarmStats = Utils.overwrittenFunction(manager.updateFarmStats, function(selfObj, superFunc, farmId, statName, value, ...)
...
function FarmlandProgressGate:consoleSetMetric(metricName, fieldName, value)
    ...
    if metricName == "" or fieldName == "" or numericValue == nil or farmId == nil then
        print("Usage: saSetMetric reputation available 100")
        return
    end

    local success, reason = self:setMetricFieldValue(farmId, metricName, fieldName, numericValue)
    if not success then
        print(string.format("%s Failed to set metric: %s", LOG_PREFIX, tostring(reason)))
        return
    end
...
function FarmlandProgressGate:consoleAddMetric(metricName, fieldName, value)
    metricName = tostring(metricName or "")
    fieldName = tostring(fieldName or "earned")
    local numericValue = tonumber(value)
    local farmId = self:getLocalFarmId()
    ...
    if metricName == "" or fieldName == "" or numericValue == nil or farmId == nil then
        print("Usage: saAddMetric reputation available 10")
        return
    end

    local success, reason = self:addMetricFieldValue(farmId, metricName, fieldName, numericValue)
    if not success then
        print(string.format("%s Failed to add metric value: %s", LOG_PREFIX, tostring(reason)))
        return
    end
...
function FarmlandProgressGate:consoleResetMetric(metricName)
    ...
    if not self:resetMetricValue(farmId, metricName) then
        print(string.format("%s Unknown metric: %s", LOG_PREFIX, metricName))
        return
    end
