...
function RandomTheftEvents.new(modDirectory)
    ...
    self.floodMaskMapId = nil
    self.floodMaskLoaded = false
    self.floodMaskSizeX = 0
    self.floodMaskSizeY = 0
...
function RandomTheftEvents:isDedicatedServerRuntime()
    return g_dedicatedServer ~= nil and g_server ~= nil and (g_client == nil or g_client.getServerConnection == nil)
end
...
function RandomTheftEvents:readGlobalEventConfig(xml, baseKey, cfg)
    ...
    if cfg == self.globalEvents.flood then
        cfg.maskFilename = self:getXMLString(xml, baseKey .. "#maskFilename", cfg.maskFilename or "")
        cfg.maskNumChannels = self:getXMLInt(xml, baseKey .. "#maskNumChannels", cfg.maskNumChannels or 1)
        cfg.maskChannel = self:getXMLInt(xml, baseKey .. "#maskChannel", cfg.maskChannel or 0)
        cfg.maskRequiredValue = self:getXMLInt(xml, baseKey .. "#maskRequiredValue", cfg.maskRequiredValue or 1)
        cfg.gridResolution = self:getXMLInt(xml, baseKey .. "#gridResolution", cfg.gridResolution or 112)
    elseif cfg == self.globalEvents.drought then
        cfg.directionCount = self:getXMLInt(xml, baseKey .. "#directionCount", cfg.directionCount or 8)
        cfg.minCoveragePercent = self:getXMLFloat(xml, baseKey .. "#minCoveragePercent", cfg.minCoveragePercent or 60)
        cfg.maxCoveragePercent = self:getXMLFloat(xml, baseKey .. "#maxCoveragePercent", cfg.maxCoveragePercent or 70)
        cfg.frontIrregularity = self:getXMLFloat(xml, baseKey .. "#frontIrregularity", cfg.frontIrregularity or 0.28)
        cfg.centerBias = self:getXMLFloat(xml, baseKey .. "#centerBias", cfg.centerBias or 0.65)
        cfg.gridResolution = self:getXMLInt(xml, baseKey .. "#gridResolution", cfg.gridResolution or 160)
    end
end
...
function RandomTheftEvents:getGlobalTargetState(eventName, fruitTypeDesc)
    ...
    if eventName == "flood" then
        return 0, "destroyedDensity"
    end

    return nil, "missing withered/disaster state"
end

function RandomTheftEvents:unloadFloodMask()
    if self.floodMaskMapId ~= nil then
        delete(self.floodMaskMapId)
        self.floodMaskMapId = nil
    end
    self.floodMaskLoaded = false
    self.floodMaskSizeX = 0
...
function RandomTheftEvents:loadFloodMask()
    self:unloadFloodMask()

    local cfg = self.globalEvents ~= nil and self.globalEvents.flood or nil
    if cfg == nil then
        return false
    end

    local maskFilename = cfg.maskFilename or ""
    if maskFilename == "" then
        self:dbg("flood mask skip: maskFilename empty")
        return false
    end

    local resolved = maskFilename
    if string.find(resolved, "$moddir$", 1, true) ~= nil then
        resolved = string.gsub(resolved, "$moddir$", self.modDirectory or THIS_MOD_DIR or "")
    else
        resolved = Utils.getFilename(resolved, self.modDirectory or THIS_MOD_DIR or "")
    end

    local mapId = createBitVectorMap("RandomTheftFloodMask")
    local numChannels = math.max(1, cfg.maskNumChannels or 1)
    local success = loadBitVectorMapFromFile(mapId, resolved, numChannels)
    if not success then
        delete(mapId)
        self:log("flood mask load failed: " .. tostring(resolved))
        return false
    end

    self.floodMaskMapId = mapId
    self.floodMaskLoaded = true
    self.floodMaskSizeX, self.floodMaskSizeY = getBitVectorMapSize(mapId)
    self:dbg(string.format("flood mask loaded file=%s size=%sx%s channels=%s", tostring(resolved), tostring(self.floodMaskSizeX), tostring(self.floodMaskSizeY), tostring(numChannels)))
    return true
end

function RandomTheftEvents:worldToMaskCoords(worldX, worldZ, mapSize)
    local maskW = self.floodMaskSizeX or 0
    local maskH = self.floodMaskSizeY or 0
    if maskW <= 0 or maskH <= 0 or mapSize == nil or mapSize <= 0 then
        return nil, nil
    end

    local nx = (worldX + mapSize * 0.5) / mapSize
    local nz = (worldZ + mapSize * 0.5) / mapSize
    nx = math.max(0, math.min(0.999999, nx))
    nz = math.max(0, math.min(0.999999, nz))
    local mx = math.floor(nx * maskW)
    local mz = math.floor(nz * maskH)
    return mx, mz
end

function RandomTheftEvents:isFloodMaskAllowedWorld(worldX, worldZ, mapSize, cfg)
    if self.floodMaskMapId == nil or not self.floodMaskLoaded then
        return false
    end
    local mx, mz = self:worldToMaskCoords(worldX, worldZ, mapSize)
    if mx == nil or mz == nil then
        return false
    end
    local channel = math.max(0, cfg.maskChannel or 0)
    local channels = math.max(1, cfg.maskNumChannels or 1)
    local value = getBitVectorMapPoint(self.floodMaskMapId, mx, mz, channel, channels)
    return value ~= nil and value >= (cfg.maskRequiredValue or 1)
end

function RandomTheftEvents:choosePercentInRange(minPercent, maxPercent)
    minPercent = tonumber(minPercent) or 0
    maxPercent = tonumber(maxPercent) or minPercent
...
function RandomTheftEvents:isGlobalFruitEligible(fruitTypeDesc)
    if fruitTypeDesc == nil or fruitTypeDesc.getDataPlaneInfo == nil then
        return false
    end

    local name = string.upper(tostring(fruitTypeDesc.name or ""))
    if name == "GRASS" or name == "MEADOW" then
        return true
    end

    return fruitTypeDesc.useForFieldMissions ~= false
end

function RandomTheftEvents:buildCellRectanglesFromGrid(grid, cellSize, mapHalf)
    local worldRects = {}
    local rows = #grid
    if rows == 0 then
        return worldRects
    ...
    for z = 1, rows do
        local cols = #grid[z]
        for x = 1, cols do
            if grid[z][x] == true then
                local x1 = -mapHalf + (x - 1) * cellSize
                local z1 = -mapHalf + (z - 1) * cellSize
                table.insert(worldRects, {x = x1, z = z1, w = cellSize, h = cellSize})
            end
        end
    end
...
function RandomTheftEvents:buildRowRectanglesFromGrid(grid, cellSize, mapHalf)
    local rects = {}
    local rows = #grid
    if rows == 0 then
        return rects
    ...
    for z = 1, rows do
        ...
        while x <= cols do
            if grid[z][x] == true then
                local startX = x
                while x + 1 <= cols and grid[z][x + 1] == true do
                    x = x + 1
                end

                local x1 = -mapHalf + (startX - 1) * cellSize
                local z1 = -mapHalf + (z - 1) * cellSize
                local width = (x - startX + 1) * cellSize
                table.insert(rects, {x = x1, z = z1, w = width, h = cellSize})
            end
            x = x + 1
        end
    end
...
function RandomTheftEvents:buildMergedRectanglesFromGrid(grid, cellSize, mapHalf)
    local rects = {}
    local rows = #grid
    if rows == 0 then
        return rects
    ...
    for z = 1, rows do
        ...
        while x <= cols do
            if grid[z][x] == true then
                ...
                table.insert(runs, {sx = startX, ex = x})
            end
            x = x + 1
        end
        ...
        local used = {}
        local nextOpen = {}
        for _, rect in ipairs(open) do
            local extended = false
            for i, run in ipairs(runs) do
                if not used[i] and rect.sx == run.sx and rect.ex == run.ex then
                    rect.ez = z
                    used[i] = true
                    table.insert(nextOpen, rect)
                    extended = true
                    break
                end
            end
        ...
        for i, run in ipairs(runs) do
            if not used[i] then
                table.insert(nextOpen, {sx = run.sx, ex = run.ex, sz = z, ez = z})
            end
        end
        open = nextOpen
    end
    ...
    for _, rect in ipairs(rects) do
        local x1 = -mapHalf + (rect.sx - 1) * cellSize
        local z1 = -mapHalf + (rect.sz - 1) * cellSize
        local width = (rect.ex - rect.sx + 1) * cellSize
        local height = (rect.ez - rect.sz + 1) * cellSize
        table.insert(worldRects, {x = x1, z = z1, w = width, h = height})
    end
    return worldRects
end

function RandomTheftEvents:buildFloodRectangles(cfg, mapSize)
    if not self.floodMaskLoaded or self.floodMaskMapId == nil then
        self:dbg("flood build skip: mask not loaded")
        return {}
    end

    local requestedResolution = math.max(64, cfg.gridResolution or 112)
    local resolution = math.min(112, requestedResolution)
    local cellSize = mapSize / resolution
    ...
    for z = 1, resolution do
        grid[z] = {}
        local worldZ = -mapHalf + (z - 0.5) * cellSize
        for x = 1, resolution do
            local worldX = -mapHalf + (x - 0.5) * cellSize
            local allowed = self:isFloodMaskAllowedWorld(worldX, worldZ, mapSize, cfg)
            grid[z][x] = allowed
            if allowed then
    ...
    if totalAllowed <= 0 then
        self:dbg("flood build skip: mask has no allowed cells")
        return {}
    end

    self:dbg(string.format("flood build mode=strictMask totalAllowed=%d res=%d requestedRes=%d", totalAllowed, resolution, requestedResolution))
    return self:buildRowRectanglesFromGrid(grid, cellSize, mapHalf)
end

function RandomTheftEvents:buildDroughtRectangles(cfg, mapSize)
    local resolution = math.max(120, cfg.gridResolution or 160)
    local cellSize = mapSize / resolution
    local mapHalf = mapSize * 0.5
    local coveragePercent = self:choosePercentInRange(cfg.minCoveragePercent or 60, cfg.maxCoveragePercent or 70) / 100
    local irregularity = math.max(0, math.min(1, cfg.frontIrregularity or 0.28))
    local centerBias = math.max(0, math.min(1, cfg.centerBias or 0.65))
    local directionCount = cfg.directionCount or 8
    if directionCount < 1 then directionCount = 8 end
    local directionIndex = math.random(1, math.min(8, directionCount))
    local vectors = {
        {1, 0}, {0.70710678, 0.70710678}, {0, 1}, {-0.70710678, 0.70710678},
        {-1, 0}, {-0.70710678, -0.70710678}, {0, -1}, {0.70710678, -0.70710678}
    }
    local attack = vectors[directionIndex]
    local ax, az = attack[1], attack[2]
    local px, pz = -az, ax
    local maxProj = mapHalf * (math.abs(ax) + math.abs(az))
    local minProj = -maxProj
    local totalDepth = (maxProj - minProj) * coveragePercent
    local startProj = maxProj - totalDepth
    local edgeAmp = totalDepth * irregularity * 0.22
    local grid = {}
    local affected = 0
    ...
    for z = 1, resolution do
        ...
        for x = 1, resolution do
            local worldX = -mapHalf + (x - 0.5) * cellSize
            local proj = worldX * ax + worldZ * az
            local lateral = (worldX * px + worldZ * pz) / maxProj
            local centerWeight = 1 - math.min(1, math.abs(lateral))
            local wave1 = math.sin((lateral + 1) * math.pi * 1.15 + directionIndex * 0.61)
            local wave2 = math.sin((lateral + 1) * math.pi * 2.45 + directionIndex * 1.17) * 0.45
            local threshold = startProj - centerWeight * centerBias * edgeAmp + (wave1 + wave2) * edgeAmp
            local active = proj >= threshold
            grid[z][x] = active
            if active then
                affected = affected + 1
            end
        ...
        for x = 1, resolution do
            ...
            for dz = -1, 1 do
                for dx = -1, 1 do
                    local nz = z + dz
                    local nx = x + dx
                    if nz >= 1 and nz <= resolution and nx >= 1 and nx <= resolution then
                        total = total + 1
                        if grid[nz][nx] then
            ...
            smoothed[z][x] = neighbors >= math.ceil(total * 0.45)
        end
    end

    self:dbg(string.format("drought build dir=%d coverage=%.2f affectedCells=%d res=%d", directionIndex, coveragePercent * 100, affected, resolution))
    return self:buildMergedRectanglesFromGrid(smoothed, cellSize, mapHalf)
end

function RandomTheftEvents:applyRectanglesToFruitType(rectangles, terrainNode, terrainDataPlaneId, startStateChannel, numStateChannels, targetState, minSourceState, maxSourceState)
    local changed = 0
    local setValue = targetState
    ...
    for _, rect in ipairs(rectangles) do
        ...
        modifier:setParallelogramWorldCoords(rect.x, rect.z, rect.w, 0, 0, rect.h, DensityCoordType.POINT_POINT_POINT)
        filter:setValueCompareParams(DensityValueCompareType.BETWEEN, minSourceState, maxSourceState)
        modifier:executeSet(setValue, filter)
        changed = changed + 1
    end
    return changed
end
...
function RandomTheftEvents:applyGlobalCropState(eventName, cfg)
    ...
    local rectangles = nil
    if eventName == "flood" then
        rectangles = self:buildFloodRectangles(cfg, mapSize)
    elseif eventName == "drought" then
        rectangles = self:buildDroughtRectangles(cfg, mapSize)
    end

    if rectangles == nil or #rectangles == 0 then
        self:dbg("global event " .. eventName .. " skip: no affected rectangles")
        return false
    end
    ...
    for _, fruitTypeDesc in pairs(fruitTypes) do
        ...
        if self:isGlobalFruitEligible(fruitTypeDesc) then
            ...
            if terrainDataPlaneId ~= nil and startStateChannel ~= nil and numStateChannels ~= nil then
                ...
                if minSourceState ~= nil and maxSourceState ~= nil then
                    local count = self:applyRectanglesToFruitType(rectangles, terrainNode, terrainDataPlaneId, startStateChannel, numStateChannels, targetState, minSourceState, maxSourceState)
                    if count > 0 then
                        processed = processed + 1
                        self:dbg(string.format("global %s fruit=%s target=%s source=%s range=%s-%s rects=%d processed", eventName, fruitName, tostring(targetState), tostring(targetSource), tostring(minSourceState), tostring(maxSourceState), count))
                    else
                        skipped = skipped + 1
                        self:dbg(string.format("global %s fruit=%s skipped reason=no matching pixels", eventName, fruitName))
                    end
                else
                    skipped = skipped + 1
                    self:dbg(string.format("global %s fruit=%s skipped reason=no valid source range", eventName, fruitName))
                end
            else
                skipped = skipped + 1
            ...
            skipped = skipped + 1
            self:dbg(string.format("global %s fruit=%s skipped reason=not eligible", eventName, fruitName))
        end
    end

    self:dbg(string.format("global %s summary processed=%d skipped=%d rects=%d", eventName, processed, skipped, #rectangles))
    return processed > 0
end
...
function RandomTheftEvents:showLocalMessage(text)
    ...
    if self:isDedicatedServerRuntime() then
        self:dbg("dialog fallback: dedicated server log only")
        self:log(text)
        return
    end
...
function RandomTheftEvents:notify(text)
    ...
    if g_server ~= nil then
        ...
        if not self:isDedicatedServerRuntime() then
