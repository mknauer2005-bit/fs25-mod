...
local function fdpError(message)
    fdpLog("error", message)
end
...
local function findNodeByNameRecursive(nodeId, targetName)
    if nodeId == nil or nodeId == 0 then
        return nil
    end

    if getName(nodeId) == targetName then
        return nodeId
    end

    local childCount = getNumOfChildren(nodeId)
    for i = 0, childCount - 1 do
        local found = findNodeByNameRecursive(getChildAt(nodeId, i), targetName)
        if found ~= nil then
            return found
        end
    end
...
local function collectFallbackEntriesFromScene()
    ...
    if getRootNode == nil then
        fdpError("getRootNode API not available")
        return collected
    end
    ...
    local dropRoot = findNodeByNameRecursive(rootNode, "FarmlandDropObjects")

    if dropRoot == nil then
        fdpError("FarmlandDropObjects root not found, fallback scene scan skipped")
        return collected
    end
...
function FarmlandDrop_onCreate(nodeId)
    ...
    if farmlandId == nil then
        fdpError(string.format("nodeId=%s skipped: missing userAttribute farmlandId", tostring(nodeId)))
        return
    end
...
function FarmlandDropRuntime:isAuthority()
    return g_server ~= nil
end
...
function FarmlandDropRuntime:onFarmlandOwnershipChanged(reason)
    if not self.initialized or not self:isAuthority() then
        return
    end
...
function FarmlandDropRuntime:installFarmlandHooks()
    if not self:isAuthority() then
        self.usePolling = false
        fdpLog("hooks", "client mode: ownership hooks disabled (server authoritative)")
        return
    end

    if g_farmlandManager == nil then
        fdpError("g_farmlandManager is nil, fallback to polling")
        self.usePolling = true
        return
    end
...
function FarmlandDropRuntime:isFarmlandOwned(farmlandId)
    ...
    if ownerFarmId == nil and self.unknownOwnerWarned[farmlandId] ~= true then
        ...
        fdpError(string.format("farmlandId=%s owner id unresolved; fallback isOwned=%s", tostring(farmlandId), tostring(farmland.isOwned)))
    end
...
function FarmlandDropRuntime:loadMap(mapName)
    ...
    if #g_farmlandDropOnCreateData == 0 then
        fdpLog("onCreate queue is empty, trying fallback scene scan by userAttribute farmlandId")
        local fallbackEntries = collectFallbackEntriesFromScene()
		if #fallbackEntries > 0 then
			fdpLog("info", "fallback populated onCreate queue")
		end
    ...
    for idx, data in ipairs(g_farmlandDropOnCreateData) do
        ...
        if self:isAuthority() and isOwnedNow and entry.applyIfOwnedOnLoad then
    ...
    fdpLog("loadMap", string.format("done entries=%d authority=%s", #self.entries, tostring(self:isAuthority())))
end
