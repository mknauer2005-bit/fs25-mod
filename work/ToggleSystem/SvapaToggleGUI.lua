...
local function stgFindDescendantByProfileRecursive(element, wantedProfile)
    if element == nil or wantedProfile == nil then
        return nil
    end
    if element.profile == wantedProfile then
        return element
    end

    local children = stgGetChildren(element) or {}
    for _, child in ipairs(children) do
        local found = stgFindDescendantByProfileRecursive(child, wantedProfile)
        if found ~= nil then
            return found
        end
    end
...
local function stgBuildToggleRowRefs(row)
    local toggleOption = stgFindDescendantByIdRecursive(row, "toggleBinaryOption")
        or stgFindDescendantByProfileRecursive(row, "svapaToggleBinaryOption")
        or stgGetChild(row, 4)

    return {
        toggleRowBackground = stgFindDescendantByIdRecursive(row, "toggleRowBackground") or stgGetChild(row, 1),
        toggleDescriptionFrame = stgFindDescendantByIdRecursive(row, "toggleDescriptionFrame") or stgGetChild(row, 2),
        toggleDescriptionText = stgFindDescendantByIdRecursive(row, "toggleDescriptionText") or stgGetChild(row, 3),
        toggleOption = toggleOption,
        toggleOptionBackground = stgFindDescendantByIdRecursive(row, "toggleOptionBackground")
            or stgFindDescendantByIdRecursive(toggleOption, "toggleOptionBackground")
            or stgFindDescendantByProfileRecursive(toggleOption, "svapaToggleBinaryBackground"),
        toggleOptionSlider = stgFindDescendantByIdRecursive(row, "toggleOptionSlider")
            or stgFindDescendantByIdRecursive(toggleOption, "toggleOptionSlider")
            or stgFindDescendantByProfileRecursive(toggleOption, "svapaToggleBinarySlider")
    }
end

local function stgScaleRowVerticalRecursive(element, scale, skipPosition)
    if element == nil or scale == nil then
        return
    end

    local width = element.size ~= nil and element.size[1] or nil
    local height = element.size ~= nil and element.size[2] or nil
    if width ~= nil and height ~= nil and element.setSize ~= nil then
        element:setSize(width, height * scale)
    end

    local posX = element.position ~= nil and element.position[1] or nil
    local posY = element.position ~= nil and element.position[2] or nil
    if not skipPosition and posX ~= nil and posY ~= nil and element.setPosition ~= nil then
        element:setPosition(posX, posY * scale)
    end

    local children = stgGetChildren(element) or {}
    for _, child in ipairs(children) do
        stgScaleRowVerticalRecursive(child, scale, false)
    end
end

local function stgApplyRowLayoutByEntry(row, entry)
    if row == nil or entry == nil then
        return
    end

    local isChildEntry = entry.parentId ~= nil and entry.parentId ~= ""
    if isChildEntry then
        stgScaleRowVerticalRecursive(row, 0.7, true)
    end
end
...
function SvapaToggleGUI:bindToggleOption(optionElement, index, entry)
    ...
    optionElement.onClickCallback = function(_, isLeftState)
        self:onClickToggleState(optionElement, isLeftState)
    end
end
...
function SvapaToggleGUI:rebuildRows()
    ...
    for index = startIndex, endIndex do
        ...
        row:setVisible(true)
        stgApplyRowLayoutByEntry(row, entry)
        ...
        if refs.toggleOption ~= nil then
            ...
            stgWarn(string.format("rebuildRows row=%d has no toggleOption element", index))
        end
...
function SvapaToggleGUI:onClickToggleState(element, state)
    ...
    SvapaToggleStateEvent.send(entry.id, newValue)
...
function SvapaToggleGUI:onClickToggleSave()
    SvapaToggleStateEvent.sendSyncRequest(true)

    if g_currentMission ~= nil and g_currentMission.addIngameNotification ~= nil then
        g_currentMission:addIngameNotification(FSBaseMission.INGAME_NOTIFICATION_INFO, "Запрос на сохранение отправлен на сервер")
    end
end
