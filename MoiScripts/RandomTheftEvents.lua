...
function RandomTheftEvents:update(dt)
    ...
    if self.elapsedSessionMinutes < self.minimumSessionMinutesBeforeEvents then
        self:dbg("periodic skip: minimum session minutes")
        return
    end

    if self.checkTimerMinutes < self.checkIntervalMinutes then
        return
    end
...
function RandomTheftEvents:tryRunStartupEvent()
    self:dbg("startup event check")
    if not self.startup.enabled then
    ...
    if self.startup.allowFuel and self.events.fuel.enabled and self.events.fuel.useStartupEvent then
    ...
    if self.startup.allowSeeds and self.events.seeds.enabled and self.events.seeds.useStartupEvent then
    ...
    if self.startup.allowFertilizer and self.events.fertilizer.enabled and self.events.fertilizer.useStartupEvent then
...
function RandomTheftEvents:tryRunStartupMissingVehicleEvent()
    if not self.startupMissingVehicle.enabled then
        return
    end
...
function RandomTheftEvents:tryRunStartupGlobalEvents()
    self:dbg("startup global event check")
    self:tryRunStartupGlobalEvent("drought", self.globalEvents.drought)
    self:tryRunStartupGlobalEvent("flood", self.globalEvents.flood)
end
...
function RandomTheftEvents:processGlobalEventsPeriodic()
    self:dbg("periodic global event check")
    self:processGlobalEventPeriodic("drought", self.globalEvents.drought)
    self:processGlobalEventPeriodic("flood", self.globalEvents.flood)
end
